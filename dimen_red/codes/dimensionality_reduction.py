...
def dimensionality_reduction(root: str,
                             data: str,
                             method: str,
                             hp: Dict[str, Any],
                             gpu: int = 0,
    ...
    if method == "pca":
        ...
        if np.max(X_train) > 1.:
            X_train = X_train/255.
        Y_train = trainds.targets.numpy() if type(
            trainds.targets) == torch.Tensor else trainds.targets

        X_test = testds.data.reshape((len(testds), -1))
        X_test = X_test.numpy() if type(X_test) == torch.Tensor else X_test
        if np.max(X_test) > 1.:
            X_test = X_test/255.
        Y_test = testds.targets.numpy() if type(
            testds.targets) == torch.Tensor else testds.targets
        ...
        X_train_pca_rescaled = (
            X_train_pca - X_train_pca.min())/(X_train_pca.max() - X_train_pca.min())
        ...
        pred_train = (X_train_pca_rescaled,
                      recons_train.reshape(img_shape), Y_train)
        pred_test = (X_test_pca_rescaled,
                     recons_test.reshape(img_shape), Y_test)
